...
const DEFAULT_SETTINGS: ActiveRecallSettings = {
    defaultSetting: "default",
};

export default class ActiveRecallPlugin extends Plugin {
    settings: ActiveRecallSettings = DEFAULT_SETTINGS;
    ...
    async onload() {
        ...
        const ribbonIconEl = this.addRibbonIcon(
            "dice",
            "Active Recall Plugin",
            (evt: MouseEvent) => {
                // Called when the user clicks the icon.
                new AppModal(this.app, this).open();
            }
        );

        // Perform additional things with the ribbon
        ribbonIconEl.addClass("active-recall-ribbon-class");

        // This adds a status bar item to the bottom of the app. Does not work on mobile apps.
        const statusBarItemEl = this.addStatusBarItem();
        statusBarItemEl.setText("Active Recall Status");
